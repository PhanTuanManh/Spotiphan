  ...
  try {
    ...
    if (!user || (user.role !== "artist" && user.role !== "admin")) {
      return res.status(403).json({
        message:
          "Unauthorized - chỉ Admin hoặc Artist có thể thực hiện hành động này.",
      });
    }
...
export const requirePremiumOrHigher = async (req, res, next) => {
  try {
    ...
    if (
      ...
      return res.status(403).json({
        message:
          "Unauthorized - chỉ Premium, Artist hoặc Admin có thể tạo Playlist.",
      });
    }
...
export const requireArtist = async (req, res, next) => {
  try {
    ...
    if (user.role !== "artist") {
      return res
        .status(403)
        .json({ message: "Unauthorized - you must be an artist" });
    }
