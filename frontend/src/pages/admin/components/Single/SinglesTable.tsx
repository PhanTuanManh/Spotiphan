...
import { Archive, CheckCircle, PackageOpen, RefreshCcw, Trash2, XCircle } from "lucide-react";
...
const SinglesTable = () => {
  const { songs, fetchAllSingles, approveSingle, unarchiveSingle,rejectSingle, archiveSingle, deleteSong,isLoading } = useSongStore();
  const [searchTerm, setSearchTerm] = useState("");
  const debouncedSearchTerm = useDebounce(searchTerm, 300);
  const [selectedSingleDelete, setSelectedSingleDelete] = useState<string | null>(null);
  const [selectedSingleArchive, setSelectedSingleArchive] = useState<string | null>(null);
  ...
  const handleApprove = async (songId: string) => {
    try {
      ...
      toast.success("Single approved successfully");
    } catch {
  ...
  const handleReject = async (songId: string) => {
    try {
      ...
      toast.success("Single rejected successfully");
    } catch {
  ...
  const handleArchive = async (songId: string) => {
    try {
      ...
      toast.success("Single archived successfully");
    } catch {
  ...
  const filteredSongs = Array.isArray(songs)
    ? songs.filter((song) => {
        ...
        return normalizedTitle.includes(normalizedSearchTerm) || normalizedArtist.includes(normalizedSearchTerm);
      })
    : [];
