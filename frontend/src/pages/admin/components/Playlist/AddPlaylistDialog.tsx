...
const AddPlaylistDialog = () => {
	...
	const handleCategorySelect = (categoryId: string) => {
		setNewPlaylist((prev) => {
			...
			return {
				...prev,
				category: isSelected
					? prev.category.filter((id) => id !== categoryId) // Bỏ chọn nếu đã chọn trước đó
					: [...prev.category, categoryId], // Thêm vào danh sách nếu chưa có
			};
		});
	};
	...
	const handleSubmit = async () => {
		...
		try {
			...
			// ✅ Không dùng JSON.stringify(), chỉ gửi mảng thuần
			newPlaylist.category.forEach((catId) => formData.append("category[]", catId));
				...
				<div className="space-y-4 py-4">
						...
						<div className="text-center">
							{image ? (
								...
								<>
									...
									<div className="text-sm text-zinc-400 mb-2">Upload playlist image</div>
					...
					<div className="space-y-2">
						...
						<Select
							...
							onValueChange={(value) => setNewPlaylist({ ...newPlaylist, isPublic: value === "true" })}
						>
					...
					{/* Category selection using custom Select */}
					<div className="space-y-2">
						...
						>
							...
							<SelectContent>
								...
								{Array.isArray(categories) && categories.map((category) => (
									<SelectItem key={category._id} value={category._id} onSelect={() => handleCategorySelect(category._id)}>
										{newPlaylist.category.includes(category._id) ? "✔ " : ""} {category.name}
									</SelectItem>
								))}
							</SelectContent>
						</Select>
